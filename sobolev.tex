...
\end{theorem}
\begin{proof}
We prove the Leibniz formula by \hl{induction} on $\abs{\alpha}.$ It is trivial to show that the equality holds for $\abs{\alpha}=0,1.$ Assuming the equality holds for for some $\alpha\neq0,$ we prove that the equality holds for $\alpha+e_i$ where is a multi-index with $1$ in the $i^\mathrm{th}$ position $\forall i\in\{1,\cdots,d\}$ and zeros elsewhere.
\eqn{
    \Der^{\al+e_i}(uv)
    &=
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Functionals, Dual Spaces, and Norms}
For the purposes of this text, a topological vector space (TVS) is a vector space for which the vector space operations of addition and scalar multiplication are continuous. That is for $x,y\in X,\,c\in\mathbb{C}$ where $X$ is some TVS, the maps $(x,y)\rightarrow x+y$ and $(c,x)\rightarrow cx$ are continuous.

A \textbf{functional} on a vector space $X$ is a scalar valued function $f:X\rightarrow \mathbb{C}.$ The functional is linear if for $x,y\in X,\,a,b\in\mathbb{C},\,f(ax+by)=af(x)+bf(y).$ If $X$ is a TVS, a functional on $X$ is continuous if it is continuous from $X$ into $\mathbb{C}$ where $\mathbb{C}$ has the usual topology induced by the Euclidean metric. The set of continuous, linear functionals on $X$ is called the \textbf{dual} of $X$ and is denoted by $X',$ which is also a vector space under pointwise addition and scalar multiplication.
...
If $X$ is a normed space and all Cauchy sequences converge in that norm, then $X$ is called a \textbf{Banach space}. A vector space $X$ is called \textbf{pre-Hilbert} if there exists a functional called the \textbf{scalar product} $\component{\cdot,\cdot}:X\times X\rightarrow\mathbb{R}$ such that $\forall u,v\in X,$
\begin{itemize}
    \item $\component{u,u}\geq 0.\,\component{u,u}=0\iff u=0$
    \item $\component{u,v}=\component{v,u}$
    \item $\component{u_1+u_2,v}=\component{u_1,v}+\component{u_2,v},\,u_1,u_2\in X$
    \item $\component{\lambda u,v}=\lambda\component{u,v},\,\lambda\in\mathbb{R}.$
\end{itemize}
The scalar product induces a norm $\norm{u}=\sqrt{\component{u,u}}.$ Another functional defined on a vector space $X$ is the \textbf{inner product} $(\cdot,\cdot):X\cross X\rightarrow \mathbb{C}$ such that for $x,y,z\in X,\, a,b\in\mathbb{C},$
\begin{itemize}
...
We define an induced norm on the dual space $X'$ of some vector space $X$ that has norm $\norm{\cdot}_X.$ For some $x'\in X',$
\eqn{
    \norm{x'}_{X'} = \sup_{x\in X,\,x\neq0}\frac{\abs{x'(x)}}{\norm{x}_{X}}
}
\begin{theorem}
...
We say $A\subsubset B$ if and only if $\overline{A}$ is a compact subset of $B.$ If, for some function $u,$ $\supp{u}\subsubset\Omega,$ we say that $u$ has \textbf{compact support} in $\Omega.$
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\end{align}
\begin{theorem}
    $C^{m,\gamm}(\Omega)$ is complete for nonnegative integer $m$ and $0<\gamma\leq1$ with respect to the norm $\norm{\cdot}_{C^{m,\gamma}(\overline{\Omega})}.$
\end{theorem}
\begin{proof}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Convolutions and Smoothing}
...
\begin{proof}
    ...
    \vspace{5em}\hl{todo}
\end{proof}
\subsection{Interpolation in $L^p(\Omega)$ Spaces}
\begin{theorem}
...
\vspace{5em}
\hl{We now consider the normed dual of $L^p(\Omega).$}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{The Spaces $W^{m,p}(\Omega)$}
