...
\eqn{
    \int_{\partial\Omega}u n_i\der S = \int_{\Omega}\p_{x_i}u\der x
}
...
\eqn{
    \int_{\partial\Omega}uvn_i\der S  &=\int_{\Omega}\p_{x_i}(uv)\der x = \int_{\Omega}u\p_{x_i}v+v\p_{x_i}v \der x
    \\
    \int_{\Omega}u\p_{x_i}v\der x &= \int_{\partial\Omega}uvn_i\der S - \int_{\Omega}v\p_{x_i}v\der x
}
...
\eqn{
    \forall\phi\in C^\infty_0(\Omega),\,\int_\Omega u\D^\al\phi\der x = (-1)^\abs{\al}\int_\Omega v\phi\der x
}
...
\begin{proof}
\eqn{
    \D^\al(J_\varepsilon*u)(x)&=\int_{\mathbb{R}^d}(\D^\al J_\varepsilon(y-x))u(y)\der y
}
