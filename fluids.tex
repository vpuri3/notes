...
\eqn{
    ...
    \lim_{\abs{h}\to0}\frac{X_i(x_0_j+h_j,t) - X_i(x_0_j,t)}{\abs{h}} = \eval{\ppp{x_j}X_i(\vect{x},t)}_{\vect{x}=\vect{x_0}}
}
...
\eqn{
    \ppp{t}\det(J) =& \ppp{t}\det\left(\ppp{x_j}X_i\right)
    = \sum_{i,j}\frac{\partial\det(J)}{\partial J_{ij}}\ppp{t}\frac{\partial X_i}{\partial x_j}
...
\todo{We now show that the stress tensor is symmetric.}
\subsection{Pressure}
...
\eqn{
    \left(\vect{x}(t+\Delta t)-\vect{x}(t)\right)\cdot\uvect{n}\der A
}
Capturing the difference with a Taylor expansion and integrating over $\partial\Omega$, we get
\eqn{
    \int_{\Omega(t+\Delta t)}\phi(\vect{x},t)\der V - \int_{\Omega(t)}\phi(\vect{x},t)\der V =
    \int_{\partial\Omega(t)} \phi(\vect{x},t)\cdot(\Delta t\vect{v_c})\cdot\uvect{n}\der A +
    \int_{\partial\Omega(t)} \phi(\vect{x},t)\cdot(\ddd{t}\vect{v}_c\dfrac{\Delta t^2}{2})\cdot\uvect{n}\der A + \mathcal{O}(\Delta t^3)
}
...
\eqn{
    ...
    &= \int_{\Omega(t)}\material\phi+\phi(\del\cdot\vect{v})\der V \\
}
...
\eqn{
    \ddd{t}\mathcal{E}
    &=\ddd{t}\int_{\Omega(t)}\frac{1}{2}\rho v_jv_j\der\vect{x}
     =\int_{\Omega(t)}\frac{1}{2}\rho\material(v_jv_j)\der\vect{x}\\
    &=\int_{\Omega(t)}\rho \vect{v}^\transp\material(\vect{v})\der\vect{x}
     =\int_{\Omega(t)}\vect{v}^\transp\left(-\grad p + \mu\del^2\vect{v}\right)\der\vect{x}\\
    &=\int_{\Omega(t)}-\del\cdot(p\vect{v}) + \mu v_jv_{j,ii} \der\vect{x}\\
    &=\int_{\Omega(t)}-\del\cdot(p\vect{v}) + \mu\left( (v_jv_{j,i})_{,i} -v_{j,i}v_{j,i} \right)  \der\vect{x}\\
    &=\int_{\partial\Omega(t)} \vect{v}\cdot\left( -p\uvect{n}+ \mu\uvect{n}\cdot\grad\vect{v} \right) \der\vect{x} - \int_{\Omega(t)}\mu v_{j,i}v_{j,i}\der\vect{x}
}
